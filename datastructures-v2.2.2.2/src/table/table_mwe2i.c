 ...
 * "owns" the payload memory, i.e., the table is responsible for
 * deallocating the payload memory. See table_mwe1i.c for a version
 * where the table borrows the memory.
 *
 * Authors: Niclas Borlin (niclas@cs.umu.se)
 *
 * Version information:
 *   v1.0  2024-04-11: First public version. Adapted from table_mwe1i.c.
 */
...
int main(int argc, char *argv[])
{
    ...
    table *t = table_empty(compare_strings, string_kill, int_kill);

    printf("Empty table from the outside:\n");
    table_print(t, print_string_int_pair);
    print_internal_with_cut_lines(t, "Empty table showing the inside structure.");
